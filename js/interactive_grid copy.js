...
function drawGrid() {
    ...
    for (let i = 0; i < gridSize; i++) {
        for (let j = 0; j < gridSize; j++) {
            ...
            const text = (i * gridSize) + j + 1;
            const textWidth = ctx.measureText(text).width;
            const padding = 5; // Adjust padding as needed
            ctx.fillStyle = 'white'; // Background color for the number
            ctx.fillRect(centerX - textWidth / 2 - padding / 2, centerY - 10, textWidth + padding, 20); // Adjust size as needed
