...
class PairedLayer4Extractor(torch.nn.Module):
    ...
    def __init__(self):
        ...
        resnet50.eval()
        self.model = torch.nn.Sequential(*(list(resnet50.children())[:-2]))
    ...
    def forward(self, inputs):
        ...
        input_concat = torch.cat((inputA[0].unsqueeze(0), inputB), dim=0)

        with torch.inference_mode():
            embeddings = self.model(input_concat)

        # Replicate the single inputA embedding N times to match inputA's shape
        self.a_embeddings = embeddings[0].unsqueeze(0).repeat(inputA.shape[0], 1, 1, 1)
        self.b_embeddings = embeddings[1:]
...
class iNaturalistPairedLayer4Extractor(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.model = resnet50_features(inat=True, pretrained=True)
        self.model.eval()
    ...
    def forward(self, inputs):
        ...
        input_concat = torch.cat((source_image[0].unsqueeze(0), target_image), dim=0)

        with torch.no_grad():
            embeddings = self.model(input_concat)

        # Replicate the single source_image embedding N times to match source_image's shape
        self.source_embedding = (
            embeddings[0].unsqueeze(0).repeat(source_image.shape[0], 1, 1, 1)
        )
        self.target_embedding = embeddings[1:]
