...
def fast4d(corr, kernel, bias=None):
    ...
    out_corr = torch.zeros((bsz, out_channels, srch, srcw, trgh, trgw)).to(device)
...
def fast6d(corr, kernel, bias, diagonal_idx):
    ...
    corr = (
        corr.permute(0, 2, 3, 1, 4, 5, 6, 7)
        .contiguous()
        .view(-1, 1, s4d, s4d, s4d, s4d)
    )
    ...
    corr = (
        corr.view(bsz, s6d, s6d, ks6d, ks6d, s4d, s4d, s4d, s4d)
        .transpose(2, 3)
        .contiguous()
        .view(-1, s6d * ks6d, s4d, s4d, s4d, s4d)
    )
    ...
    first_sum = (
        torch.stack(first_sum)
        .transpose(0, 1)
        .view(bsz, s6d * ks6d, ndiag, s4d, s4d, s4d, s4d)
    )
    ...
    corr = (
        torch.stack(corr)
        .transpose(0, 1)[:, sidx:eidx, sidx:eidx, :, :, :, :]
        .unsqueeze(1)
        .contiguous()
    )
    corr += bias.view(1, -1, 1, 1, 1, 1, 1, 1)

    reverse_idx = torch.linspace(s6d * s6d - 1, 0, s6d * s6d).long().to(device)
    corr = corr.view(bsz, 1, s6d * s6d, s4d, s4d, s4d, s4d)[
        :, :, reverse_idx, :, :, :, :
...
def init_param_idx4d(param_dict):
    ...
    for key in param_dict:
        ...
        param_idx.append(torch.tensor(param_dict[key]).to(device))
    return param_idx
...
class CHM4d(_ConvNd):
    ...
    def __init__(self, in_channels, out_channels, ksz4d, ktype, bias=True):
        ...
        self.zero_kernel4d = torch.zeros(
            (in_channels, out_channels, ksz4d, ksz4d, ksz4d, ksz4d)
        ).to(device)
        ...
        if self.param_idx is None:
            ...
            for idx, pdx in enumerate(self.param_idx):
                ...
                for jdx, kernel_single in enumerate(kernel):
                    weight = self.weight[idx + jdx * len(self.param_idx)].repeat(
                        len(pdx)
                    ) / len(pdx)
                    kernel_single.view(-1)[pdx] += weight
            kernel = kernel.view(
                self.in_channels, self.out_channels, ksz, ksz, ksz, ksz
            )
        return kernel
...
class CHM6d(_ConvNd):
    ...
    def __init__(self, in_channels, out_channels, ksz6d, ksz4d, ktype):
        ...
        # Zero kernel initialization
        self.zero_kernel4d = torch.zeros((ksz4d, ksz4d, ksz4d, ksz4d)).to(device)
        self.zero_kernel6d = torch.zeros((ksz6d, ksz6d, ksz4d, ksz4d, ksz4d, ksz4d)).to(
            device
        )
        self.nkernels = in_channels * out_channels
        ...
        self.diagonal_idx = [
            torch.tensor(x).to(device) for x in [[6], [3, 7], [0, 4, 8], [1, 5], [2]]
        ]
        param_dict4d = chm_kernel.KernelGenerator(ksz4d, ktype).generate()
        param_shared = param_dict4d is not None
        ...
        if param_shared:  # psi & iso kernel initialization
            ...
            self.param_dict6d = [torch.tensor(i).to(device) for i in self.param_dict6d]
        ...
        Logger.info(
            "(%s) # params in CHM 6D: %d"
            % (ktype, sum([len(x.view(-1)) for x in self.param]))
        )
        self.weight = None
    ...
    def init_kernel(self):
        ...
        for idx, (param, param_dict6d) in enumerate(zip(self.param, self.param_dict6d)):
            ...
            kernel6d.view(-1, ksz4d, ksz4d, ksz4d, ksz4d)[
                param_dict6d
            ] += kernel4d.view(ksz4d, ksz4d, ksz4d, ksz4d)
